...
| 四宫格/多参考图 | sharp 切图、`@图片N` | FFmpeg Media 预处理 + Provider 有序多参考图 | 已验证 |
...
## 第七条纵向切片：组合参考图与有序多图生成

1. `CreativeReferenceGridRequest/Result` 有 Zod 契约与 JSON Schema，网格限制为 1×2 至 3×3、总数 2～9。
2. 媒体层使用已有 FFmpeg 一次解码，按行优先顺序裁切 PNG，不引入 Sharp 或第二套图片服务。
3. 每张切片拥有确定性资产 ID、内容 SHA-256、原图 `parentAssetId`、行列标记与受控 MinIO URI。
4. 切片资产登记、当前实体参考替换、乐观版本递增和审计在同一 PostgreSQL 事务中完成；重放幂等，过期版本返回 409。
5. 图片生成按上一镜尾帧、本镜首帧、分镜、角色、场景、道具的固定优先级收集主/补充参考，去重后与 `@图片N` 映射严格对齐。
6. 实跑将受控 2×2 源图拆成 4 张 PNG 并绑定角色 v2；同键回放不新增资产记录，后续 Mock 图片 Job 收到 7 张有序参考与 7 条语义映射。
7. React 创作台支持选源图、网格、拆分并绑定；桌面与 `390 × 844` 无横向溢出，控制台无错或警告。
