...
| 全局素材库 | 角色/场景/道具复用与去重 | PostgreSQL 检索 + 项目本地资产血缘别名 | 已验证 |
...
| 工作流分组 | 框选分镜、组合步骤、整组重跑 | PostgreSQL 镜头组 + 现有持久批次/BullMQ | 已验证 |
...
1. 全局检索只暴露未归档、已登记且使用受控 `s3://` URI 的图片类资产，可按关键词、实体类型和排除项目查询。
2. 搜索结果携带原工程与原角色/场景/道具上下文，并按内容 SHA-256 去重；已复用的本地别名不会被当成新来源形成链式复制。
3. 跨工程复用不重复上传媒体；目标项目生成确定性 `asset_reuse_*` 别名，保留原 URI、内容哈希、许可证、Provider/Model 与来源 `parentAssetId`。
4. 别名登记、实体参考绑定、乐观版本递增和审计在同一 PostgreSQL 事务中完成；相同键回放幂等，过期版本返回 409。
5. 目标项目导出依然会按本地资产记录将媒体收入工程包，不需要在导入端保留原工程的可编辑状态。
6. 实跑从另一真实项目搜索并复用林遥角色图；目标角色从 v1 升到 v2，本地别名与原资产共用 URI/SHA-256 并保留血缘，同键回放返回原结果。
7. React 创作台提供全局搜索、来源工程/用途展示、复用和已复用状态；桌面与 `390 × 844` 无横向溢出，控制台无错或警告。

## 第九条纵向切片：持久化镜头工作流组

1. `CreativeWorkflowGroup`、创建请求和运行请求都有 Zod 契约与 JSON Schema；组内镜头不重复，数量限制为 1～100，并记录生成类型、缺失项策略和并发度。
2. 镜头组持久化在 PostgreSQL，创建时拒绝缺失镜头与跨项目镜头，工程内名称唯一；确定性组 ID 支持相同幂等请求回放，不同输入复用同键返回冲突。
3. 每次运行校验镜头组乐观版本与组内所有分镜版本，过期请求返回 409；成功后把最新批次 ID 与组版本原子关联，页面刷新后可恢复。
4. 运行只组装现有 `CreativeGenerationBatchRequest`，复用持久批次、Provider Gateway、预算闸门、BullMQ 子 Job、停止/重试和资产版本链，不引入第二套编排器或审批入口。
5. “补齐缺失项”继承组的 `missingOnly` 策略，“整组生成新版本”仅将本次请求切换为强制重跑，不修改保存的默认策略。
6. 真实 API 实跑证明已有图片被标记为 `skipped`；强制重跑创建新批次，并在项目预算不足时进入现有飞书人工闸门，镜头组版本从 v1 递增到 v3。
7. React 创作台支持逐镜勾选、选择当前集、保存、补齐与整组重跑；浏览器直接创建后刷新仍能读回，桌面与 `390 × 844` 无横向溢出，控制台无错误或警告。
