...
| 创作工作台 | 剧本编辑、分镜列表/画布、提示词与连续性编辑、素材库和成片审阅 | 已实现并实操验证 |
| 工程导入导出 | 导入结构化 JSON 或含媒体的 ZIP；导出带 SHA-256 完整性校验的 OneCrew 工程包 | 已实现并测试 |
| 并发编辑 | 剧集/分镜乐观版本、409 冲突防护、成功编辑审计 | 已实现并测试 |
...
5. 保存时界面会携带当前版本。如果其他编辑者已先保存，过期修改会收到 HTTP 409，界面随即重新读取最新内容，不会静默覆盖；
6. 点击“导出 OneCrew 工程”下载可再导入的完整 ZIP。包内含项目、剧集、素材实体、分镜、帧提示词、版本化资产和每个媒体文件的 SHA-256。
...
- 创作台已在桌面与 `390 × 844` 移动视口验证，项目切换、剧本/分镜保存、版本冲突、画布、导出和成片审阅可用，干净会话浏览器控制台无错误或警告；
