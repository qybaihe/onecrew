...
15. 点击“导出 OneCrew 工程”下载可再导入的完整 ZIP。包内含项目、剧集、素材实体、分镜、帧提示词、版本化资产和每个媒体文件的 SHA-256。
...
| 全局素材搜索/复用 | `GET /v1/creative/reusable-assets`；`POST /v1/creative/entities/:entityId/reusable-assets` |
...
| 镜头工作流组 | `GET/POST /v1/creative/projects/:projectId/workflow-groups`；`POST /v1/creative/workflow-groups/:groupId/run` |
...
- 全局素材库已用两个真实数据库工程实跑：搜索结果返回原工程/实体与受控 MinIO URI，复用后目标角色升到 v2，项目本地别名保留原 URI、SHA-256 和 `parentAssetId`，同键回放幂等；
- 镜头工作流组已用真实数据库与浏览器实跑：勾选并保存后刷新页面仍可恢复；缺失项运行跳过已有图片，强制重跑创建新批次并在预算不足时进入现有飞书人工闸门；
- 创作台已在桌面与 `390 × 844` 移动视口验证，故事规划预览/写入、项目切换、剧本/分镜保存、角色/场景/道具编辑、组合参考图拆分与绑定、全局素材搜索与复用、镜头工作流组、版本资产绑定、单镜与批量图片/视频生成、批次恢复/停止/重试、连续性 QC、版本链回写、版本冲突、画布、导出和成片审阅可用，干净会话浏览器控制台无错误或警告；
