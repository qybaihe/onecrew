...
export function ReviewApp({ onOpenStudio }: ReviewAppProps) {
  ...
  return (
    <div className="shell">
      <header className="topbar">
        <a className="brand" href="#review" aria-label="OneCrew 审片页首页">
          <span className="brand-mark" aria-hidden="true">✦</span>
          <span><strong>ONECREW</strong><small>REMOTION REVIEW</small></span>
        </a>
        <div className="topbar-actions">
          <button className="mode-switch" type="button" onClick={onOpenStudio}>返回创作台</button>
          <div className="read-only"><span aria-hidden="true" /> 只读审片</div>
        </div>
      </header>

      <main id="preview" className="workspace">
        <aside className="rail" aria-label="Composition 列表">
          <div className="rail-heading"><p>DELIVERABLES</p><h1>成片规格</h1></div>
          <nav className="composition-list">
            {compositionOptions.map((option, index) => (
              ...
              >
                ...
                <span className="composition-copy"><small>{option.eyebrow}</small><strong>{option.label}</strong></span>
        ...
        <section className="stage" aria-labelledby="stage-title">
          <div className="stage-heading">
            <div><p>{selected.eyebrow} · {manifest.locale}</p><h2 id="stage-title">{manifest.localePack.title}</h2></div>
